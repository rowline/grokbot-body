...
必须用绑定页里那条带密钥的地址认领和调用。旧的 `/mcp` 没有密钥，认领会被拒绝。Connector 是账号级的，所有 Bot 都能看见；锁是配对码加上连接地址里的密钥。猜码太勤会被暂时挡住。
...
默认「按住说话」：按住球体说话，松手发给绑定的 Bot。办公室用这个。安静环境可在绑定页切到「一直听」——周围说话也会收进去。「按住录像」松手存到相册。绑定页「任务完成出声」默认开，拍完转完只说一句短的；长内容写在对话框。

听到的原话和 Bot 的回答应写进 Grok Bot 对话框。手机默认用系统声，自动挑已下载的高级/增强中文音色（雨舒、力穆比压缩版婷婷自然）。绑定页可选、可试听。压缩音色听着机械：去 iPhone 设置 → 辅助功能 → 朗读内容 → 声音 → 中文，下载增强或高级。绑定页可贴自己的 xAI 密钥，开口才换成 Grok 音色（Eve / Ara / Leo / Rex / Sal）。密钥留在手机，不过云。没有密钥就没有 Grok 声。
...
绑定页「脸」可以选形状（圆、卵石、方圆、胶囊、三角、六边、云、水滴）、颜色，和休息表情（平静、开心、生气、困倦等 16 种）。思考绕彩带，出错变成感叹号，`hex` 变成六边形。Bot 换脸会暂时盖过休息表情。

变形跟 [bloub](https://github.com/jeremy-prt/bloub) 一样：径向轮廓、指数 ease-out，身体不过冲。手指按在屏幕上搓它，眼睛跟着看，身体被按扁；松手缓回来。眼睛跟着手机倾斜。轻轻晃一下会愣住；故意连着用力晃会发晕，大约两秒后自己好。
